...
namespace RibbonControl.Samples.MvvmOnly.ViewModels;
...
public class MainWindowViewModel : RibbonObservableObject
{
    ...
    public MainWindowViewModel()
    {
        ...
        foreach (var tab in BuildWordTabs())
        {
            Ribbon.Tabs.Add(tab);
        }
        ApplyIconCustomizationSamples(Ribbon.Tabs);

        TogglePictureToolsCommand = new RelayCommand(_ => TogglePictureTools());
        ToggleAddInsTabCommand = new RelayCommand(_ => ToggleAddInsTab());
        ToggleMinimizedCommand = new RelayCommand(_ => Ribbon.IsMinimized = !Ribbon.IsMinimized);
        ToggleKeyTipModeCommand = new RelayCommand(_ => Ribbon.IsKeyTipMode = !Ribbon.IsKeyTipMode);
    ...
    {
        var existing = Ribbon.Tabs.FirstOrDefault(x => string.Equals(x.Id, "add-ins", StringComparison.Ordinal));
        if (existing is not null)
        {
            Ribbon.Tabs.Remove(existing);
            Status = "Add-ins tab removed";
            return;
        }

        Ribbon.Tabs.Add(BuildAddInsTab());
        Status = "Add-ins tab added";
    }
    ...
    private static IEnumerable<RibbonTabViewModel> BuildWordTabs()
    {
        yield return BuildHomeTab();
        yield return BuildInsertTab();
        yield return BuildLayoutTab();
        yield return BuildReferencesTab();
        yield return BuildMailingsTab();
        yield return BuildReviewTab();
        yield return BuildViewTab();
        yield return BuildHelpTab();
        yield return BuildPictureTab();
    }
    ...
    private static RibbonTabViewModel BuildInsertTab()
        => Tab(
            "insert",
            "Insert",
            1,
            ...
            Group("illustrations", "Illustrations", 2,
                Item("picture", "Picture", FluentIconData.Image20Regular, 0, "picture", "P", "Insert a picture"),
                Item("shapes", "Shapes", FluentIconData.Settings20Regular, 1, "shapes", "SP", "Insert a shape"),
                Item("chart", "Chart", FluentIconData.ChartMultiple20Regular, 2, "chart", "H", "Insert a chart")),
            Group("links", "Links", 3,
    ...
    private static RibbonTabViewModel BuildLayoutTab()
        => Tab(
            "layout",
            "Layout",
            2,
            Group("page-setup", "Page Setup", 0,
                Item("margins", "Margins", FluentIconData.Settings20Regular, 0, "margins", "MG", "Set page margins"),
                Item("orientation", "Orientation", FluentIconData.ChartMultiple20Regular, 1, "orientation", "OR", "Set page orientation"),
                Item("size", "Size", FluentIconData.Copy20Regular, 2, "size", "SZ", "Set paper size")),
    ...
    private static RibbonTabViewModel BuildReferencesTab()
        => Tab(
            ...
            Group(
                ...
                2,
                RibbonGroupDockedCenterLayoutMode.Vertical,
                Item("table-contents", "Insert Table of Contents", FluentIconData.DocumentBulletList20Regular, 0, "table-contents", "TC", "Insert table of contents", layoutDock: RibbonItemLayoutDock.Left),
                Item("update-table-contents", "Update Table of Contents", FluentIconData.ArrowSync20Regular, 1, "update-table-contents", "UT", "Refresh the generated table", size: RibbonItemSize.Small, layoutDock: RibbonItemLayoutDock.Center),
                Item("remove-table-contents", "Remove Table of Contents", FluentIconData.ArrowUndo20Regular, 2, "remove-table-contents", "RT", "Remove the generated table", size: RibbonItemSize.Small, layoutDock: RibbonItemLayoutDock.Center)),
            Group(
                "footnotes",
                "Footnotes",
                1,
                ...
                3,
                RibbonGroupDockedCenterLayoutMode.Vertical,
                Item("insert-footnote", "Insert Footnote", FluentIconData.TextFootnote20Regular, 0, "insert-footnote", "FN", "Insert footnote", layoutDock: RibbonItemLayoutDock.Left),
                Item("insert-endnote", "Insert Endnote", FluentIconData.DocumentEndnote20Regular, 1, "insert-endnote", "EN", "Insert endnote", size: RibbonItemSize.Small, layoutDock: RibbonItemLayoutDock.Center),
            ...
            Group("insights", "Insights", 2,
                Item("references-search", "Search", FluentIconData.Search20Regular, 0, "find", "RS", "Search references and related topics")),
            Group("citation-bibliography", "Citation & Bibliography", 3,
                MenuButtonItem("citations", "Citations", FluentIconData.Grid20Regular, 0, "citations", "CN", "Insert and manage citations",
                    MenuEntryDetailed("citations-insert", "Insert Citation", 0, "citations-insert", FluentIconData.Grid20Regular),
                    MenuEntryDetailed("citations-manage-sources", "Manage Sources", 1, "citations-manage-sources", FluentIconData.Wrench20Regular),
                    MenuEntryDetailed("citations-style-apa", "Style: APA", 2, "citations-style-apa", FluentIconData.Document20Regular))));
    ...
    private static RibbonItemViewModel ToggleItem(
        string id,
        string label,
        int order,
        string commandId,
