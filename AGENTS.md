...
- Prefer composition in ViewModels/services/code over inheritance wherever possible,
  except where framework base types are required for explicitly selected sample stacks.
...
- All control themes, templates, and visual resource dictionaries must be authored
  in XAML (no C#-constructed themes/templates).
- Author control templates via `ControlTheme` resources (for example
  `{x:Type MyControl}` keys), not `Style` selectors with `Template` setters.
...
## 4) ReactiveUI (samples-only, opt-in elsewhere)

Reference: https://github.com/reactiveui/ReactiveUI

### Usage policy
- ReactiveUI is allowed by default in sample projects only.
- Do not introduce ReactiveUI dependencies into core libraries, infrastructure,
  or test projects unless the user explicitly requests it.
- If ReactiveUI is requested outside samples, isolate usage behind abstractions
  and keep non-ReactiveUI implementation paths available.

### Navigation (ReactiveUI routing)
- For sample projects that choose ReactiveUI, use `IScreen` with a single
  `RoutingState` as the navigation root.
- For sample projects that choose ReactiveUI, all navigable ViewModels implement
  `IRoutableViewModel`.
- For sample projects that choose ReactiveUI, views host navigation via
  `RoutedViewHost`.
- Use route segments that are stable, explicit, and testable.

### Avalonia integration
- In sample projects using ReactiveUI, use `ReactiveUI.Avalonia` (latest) and do
  not use `Avalonia.ReactiveUI` directly.
...
- Use Dock.Avalonia for the view layer and Dock.Model.ReactiveUI for MVVM
  integration only in sample projects that explicitly use ReactiveUI.
...
- Every `.cs` source file must start with this exact header:
...
- In sample apps, define and consume icon constants (for example `FluentIconData`) instead of inline SVG strings in XAML or ViewModels.
